...
import { FileCheck, BookOpen, Download, FolderOpen } from "lucide-react";
...
import { detectProductFamily } from "../data/productMatchers";
import type { SizingResult } from "../types/shared";

// ─── Sub-tab type ────────────────────────────────────────────
type DocSubTab = "datasheet" | "qap" | "master";
...
export default function DocumentPanel({ sizingResults }: DocumentPanelProps = {}) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ═══ Content — CSS display instead of conditional render to preserve state ═══ */}
      <div style={{ display: subTab === "datasheet" ? "block" : "none" }}>
        <DatasheetGeneratorPanel onDatasheetChange={handleDatasheetChange} />
      </div>
      <div style={{ display: subTab === "qap" ? "block" : "none" }}>
        <QapGeneratorPanel />
      </div>
      <div style={{ display: subTab === "master" ? "block" : "none" }}>
