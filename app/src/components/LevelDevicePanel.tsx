...
export default function LevelDevicePanel({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ═══ FLOAT MOC SELECTOR — Side & Top Mounted Magnetic Level Gauge ONLY ═══ */}
      {(() => {
        const magneticDevices = [...bestDevices, ...suitableDevices].filter(
          (rec) => rec.device.type === "side_mounted_magnetic" || rec.device.type === "top_mounted_magnetic"
        );
        return magneticDevices.length > 0 ? (
          <div>
            <button
            ...
            >
              ...
              <span className="ml-auto text-[10px] font-normal opacity-70">
                {magneticDevices.length} Magnetic Level Gauge{magneticDevices.length > 1 ? "s" : ""} recommended
              </span>
