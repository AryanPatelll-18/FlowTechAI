...
import { FileText, Upload, Printer, Download, Sparkles, Eraser, Eye, Table, Crosshair, AlertCircle, CheckCircle, FolderOpen, Calculator, Pencil } from "lucide-react";
import SizingCheckPanel from "./SizingCheckPanel";
import GadAnnotationEditor from "./GadAnnotationEditor";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { generateFromSOText, generateFromDatasheet, openPrintWindow, downloadHtml, type SODatasheet } from "../data/api";
import { readFileAsText } from "../data/pdfExtractor";
import { getDrawingsForFamily, getDrawingUrlById } from "../data/gaDrawingLookup";
...
interface Props {
  ...
  sizingResults?: import("../types/shared").SizingResult[];
}

export default function DatasheetGeneratorPanel({ onDatasheetChange, sizingResults }: Props = {}) {
  ...
  const [showSizingCheck, setShowSizingCheck] = useState(false);
  const [inlineAnnotate, setInlineAnnotate] = useState<{ drawing: GaDrawingEntry; dataUrl: string } | null>(null);
        ...
        <div className="space-y-3">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {ds.lineItems.map((it, idx) => (
                <button key={idx} onClick={() => { setEditingIdx(editingIdx === idx ? null : idx); setDrawingSelectorOpen(false); setAvailableDrawings([]); }} className={`text-[9px] px-2 py-1 rounded border transition-all text-left ${editingIdx === idx ? "bg-red-600 text-white border-red-600" : "bg-gray-50 border-gray-200 hover:border-red-300"}`}>
                  <div className="flex items-center gap-1.5">
                    <span>{it.srNo}. {it.tagNo}</span>
                    <span className="opacity-70">({it.instrumentType})</span>
                    {it.decodNo && <span className={`text-[8px] px-1 rounded ${editingIdx === idx ? "bg-white/20" : "bg-blue-100 text-blue-700"}`}>{it.decodNo}</span>}
                  </div>
                </button>
              ))}
            </div>
          ...
          {editingIdx !== null && ds.lineItems[editingIdx] && (
            <div className="bg-white border border-red-200 rounded-lg p-4">
              ...
              {/* De-Codification No. — MANDATORY display */}
              {ds.lineItems[editingIdx].decodNo && (
                <div className="mb-3 bg-blue-50 border border-blue-200 rounded px-3 py-1.5 flex items-center gap-2">
                  <span className="text-[9px] font-bold text-blue-700">De-Codification No.:</span>
                  <span className="text-[10px] font-mono text-blue-900 font-bold">{ds.lineItems[editingIdx].decodNo}</span>
                </div>
              )}
              ...
              {inlineAnnotate && (
                <div className="mt-3 border border-purple-200 rounded-lg overflow-hidden" style={{ height: "500px" }}>
                  <GadAnnotationEditor
                    imageUrl={inlineAnnotate.dataUrl}
                    drawingNo={inlineAnnotate.drawing.drawingNo}
                    title={inlineAnnotate.drawing.title}
                    onSave={(dataUrl) => {
                      // Update the selected drawing's data URL with annotated version
                      const updated = { ...ds };
                      updated.lineItems[editingIdx] = {
                        ...updated.lineItems[editingIdx],
                        gaDrawingUrl: dataUrl,
                      };
                      setDs(updated);
                      setInlineAnnotate(null);
                    }}
                    onCancel={() => setInlineAnnotate(null)}
                  />
                </div>
              )}
              ...
              <div className="mt-3 border border-blue-200 rounded-lg overflow-hidden">
                ...
                {ds.lineItems[editingIdx].gaDrawingUrl && !drawingSelectorOpen && (
                  <div className="p-2 text-[8px] text-gray-600 bg-white">
                    ...
                    {/* Quick annotate button for selected drawing */}
                    {availableDrawings.length > 0 && (() => {
                      const selected = availableDrawings.find((o) => o.dataUrl === ds.lineItems[editingIdx].gaDrawingUrl);
                      return selected ? (
                        <button
                          onClick={() => setInlineAnnotate({ drawing: selected.drawing, dataUrl: selected.dataUrl })}
                          className="mt-1.5 text-[8px] px-2 py-0.5 bg-purple-50 text-purple-700 rounded border border-purple-200 hover:bg-purple-100 transition-colors flex items-center gap-1"
                        >
                          <Pencil className="w-2.5 h-2.5" /> Add dimension annotations
                        </button>
                      ) : null;
                    })()}
                  </div>
          ...
          <div className="flex items-center justify-end gap-2">
            <Button variant="outline" size="sm" onClick={() => setShowSizingCheck(!showSizingCheck)} className="text-[10px] h-7"><Calculator className="w-3 h-3 mr-1" />{showSizingCheck ? "Hide" : "Sizing Check"}</Button>
          ...
          {showSizingCheck && (
            <SizingCheckPanel
              instruments={ds.lineItems.map((it) => ({
                srNo: it.srNo,
                tagNo: it.tagNo,
                instrumentType: it.instrumentType,
                size: it.size,
                processData: it.processData,
              }))}
              soNo={ds.header.soNo}
              clientName={ds.header.client}
              projectName={ds.header.project}
            />
          )}
        </div>
      )}
