...
function toM3hr(flowRate: number, unit: string, density?: number): number {
  ...
  // Normalize unit: lowercase, remove spaces, convert superscript 3 to regular 3
  const u = (unit || "").trim().toLowerCase().replace(/\s/g, "").replace(/³/g, "3");
  switch (u) {
    case "m3/hr": return flowRate;
    case "lph": case "l/hr": return flowRate / 1000;
    case "lpm": return flowRate * 0.06;
    case "lps": return flowRate * 3.6;
    case "gpm": return flowRate * 0.227125;
    case "gph": return flowRate * 0.003785;
    case "cfm": return flowRate * 1.699;
    case "kg/hr": return density && density > 0 ? flowRate / density : 0;
    case "nm3/hr": return density && density > 0 ? (flowRate * 1.205) / density : flowRate;
...
export function generateUncertaintyBudgetChart(data: SizingReportData): string {
  try {
    ...
    budget.forEach((item, i) => {
      ...
      // Type badge
      svg += `<text x="${m.l-240}" y="${y+barH/2+3}" text-anchor="middle" font-size="7" fill="${item.type === 'A' ? '#1a73e8' : '#5f6368'}" font-weight="bold">${item.type}</text>`;

      // Stated value
      svg += `<text x="${m.l-190}" y="${y+barH/2+3}" text-anchor="middle" font-size="7" fill="#666">${item.statedValue}</text>`;

      // Distribution
      svg += `<text x="${m.l-110}" y="${y+barH/2+3}" text-anchor="middle" font-size="7" fill="#888">${item.distribution}</text>`;

      // Label (source name) — placed last to avoid overlap with other columns
      svg += `<text x="${m.l-10}" y="${y+barH/2+3}" text-anchor="end" font-size="8" fill="#444">${item.symbol} ${item.source}</text>`;
    ...
    // Column headers
    svg += `<text x="${m.l-240}" y="${H-m.b+18}" text-anchor="middle" font-size="8" fill="#333" font-weight="bold">Type</text>`;
    svg += `<text x="${m.l-190}" y="${H-m.b+18}" text-anchor="middle" font-size="8" fill="#333" font-weight="bold">Stated</text>`;
    svg += `<text x="${m.l-110}" y="${H-m.b+18}" text-anchor="middle" font-size="8" fill="#333" font-weight="bold">Distribution</text>`;
    svg += `<text x="${m.l-50}" y="${H-m.b+18}" text-anchor="start" font-size="8" fill="#333" font-weight="bold">Source</text>`;
    svg += `<text x="${m.l+60}" y="${H-m.b+18}" text-anchor="middle" font-size="8" fill="#333" font-weight="bold">Standard Uncertainty uᵢ (%)</text>`;
...
export function buildSizingReportData(
  ...
  let velMin = areaM2 > 0 ? (qMinM3hr / 3600) / areaM2 : 0;
  let velMax = areaM2 > 0 ? (qMaxM3hr / 3600) / areaM2 : 0;
  let velNorm = areaM2 > 0 ? (qNormM3hr / 3600) / areaM2 : 0;

  // Fallback: if velocities are 0 but flow rates are non-zero, recalculate
  // using a standard pipe area lookup based on the SO specified size
  if ((velMin === 0 && flowMin > 0) || (velMax === 0 && flowMax > 0)) {
    const soSize = s.soSpecifiedSize || s.pipeSize || pipeKey;
    const soPipe = soSize ? (PIPE_DIMENSIONS[soSize] || null) : null;
    const fallbackArea = soPipe ? soPipe.crossSectionalAreaM2 : areaM2;
    if (fallbackArea > 0) {
      if (velMin === 0 && flowMin > 0) velMin = (qMinM3hr / 3600) / fallbackArea;
      if (velMax === 0 && flowMax > 0) velMax = (qMaxM3hr / 3600) / fallbackArea;
      if (velNorm === 0 && qNorm > 0) velNorm = (qNormM3hr / 3600) / fallbackArea;
    }
  }
  ...
  return {
    ...
    meterModel: s.modelName || product.model || r.meterModel || "—",
...
export function buildSizingReportDataForSize(
  ...
  // ── Handle size input: can be a STRING (e.g. "80NB") or an OBJECT ──
  // When called from ProjectSizingRunner, size is often the SO string.
  // Find the matching size object from result.sizes, or build one.
  const soSizeStr = typeof size === "string" ? size : (size && size.size ? size.size : "");
  let sz: any = {};
  if (typeof size === "object" && size !== null) {
    sz = size;
  } else if (r.sizes && Array.isArray(r.sizes)) {
    sz = r.sizes.find((siz: any) => siz.size === soSizeStr || siz.size === size) || {};
  }
  const meterSize = sz.size || soSizeStr || r.bestSize || "—";

  // ── Derive pipe size from SO size (e.g. "80NB" → "DN80") ──
  const pipeKey = (() => {
    const soPipe = soSizeStr || s.pipeSize || s.pipeSizeNominal || "";
    // Try direct lookup first
    if (soPipe && PIPE_DIMENSIONS[soPipe]) return soPipe;
    // Convert "80NB" → "DN80"
    const numMatch = soPipe.match(/(\d+)/);
    if (numMatch) {
      const dnKey = `DN${numMatch[1]}`;
      if (PIPE_DIMENSIONS[dnKey]) return dnKey;
    }
    return s.pipeSizeNominal || "DN80";
  })();
  const pipe = PIPE_DIMENSIONS[pipeKey] || PIPE_DIMENSIONS["DN80"];
  const areaM2 = pipe ? pipe.crossSectionalAreaM2 : 0.00477;
  ...
  // ── Calculate velocities DIRECTLY from flow rate + pipe area ──
  // ( sizing result may not have velocityAtQmin/velocityAtQmax )
  const qMinM3hr = toM3hr(flowMin, flowUnit, density);
  const qMaxM3hr = toM3hr(flowMax, flowUnit, density);
  const qNormM3hr = toM3hr(qNorm, flowUnit, density);
  ...
  const sizingNotes: string[] = [];
  sizingNotes.push(`${product.name || "Flow Meter"} — Size: ${meterSize}`);
  if (r.reason) sizingNotes.push(r.reason);
  ...
  if (result.bestSize === meterSize) sizingNotes.push("BEST MATCH — recommended size");
  ...
  return {
    ...
    clientName: clientName || s.clientName || "—",
    soRef: soRef || s.soRef || "—",
    date: new Date().toLocaleDateString("en-GB"),
    preparedBy: "Flowtech AI Sizing Tool",
    service,
    fluidName: s.selectedLiquid?.name || s.selectedGas?.name || s.fluidName || service,
    fluidDensity: density,
    fluidViscosity: visc,
    operatingTemp: s.operatingTemp || 20,
    operatingPressure: opPressure,
    pipeSizeNominal: soSizeStr || pipeKey,
    qMin: flowMin, qMax: flowMax, qNormal: qNorm, flowUnit,
    velocityMin: velMin, velocityMax: velMax, velocityNormal: velNorm,
    reynoldsMin: reMin, reynoldsMax: reMax,
    turndownRatio: flowMax / Math.max(flowMin, 0.001),
